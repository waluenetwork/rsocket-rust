...
#[repr(C)]
pub struct GoRSocketPayload {
    pub inner: Box<Payload>,
}

#[no_mangle]
pub extern "C" fn rsocket_go_payload_create(
    data: *const u8,
    data_len: size_t,
    metadata: *const u8,
    metadata_len: size_t,
) -> *mut GoRSocketPayload {
    ...
    Box::into_raw(Box::new(GoRSocketPayload {
        inner: Box::new(payload),
    }))
}

#[no_mangle]
pub extern "C" fn rsocket_go_payload_create_from_string(
    data: *const c_char,
...
#[no_mangle]
pub extern "C" fn rsocket_go_payload_get_data_length(payload: *const GoRSocketPayload) -> size_t {
    if payload.is_null() {
        return 0;
    }
    unsafe {
        (*payload).inner.data().map_or(0, |data| data.len())
    }
}

#[no_mangle]
pub extern "C" fn rsocket_go_payload_get_metadata_length(payload: *const GoRSocketPayload) -> size_t {
    if payload.is_null() {
        return 0;
    }
    unsafe {
        (*payload).inner.metadata().map_or(0, |metadata| metadata.len())
    }
}

#[no_mangle]
pub extern "C" fn rsocket_go_payload_copy_data(
    payload: *const GoRSocketPayload,
    buffer: *mut u8,
    buffer_len: size_t,
) -> size_t {
...
#[no_mangle]
pub extern "C" fn rsocket_go_payload_copy_metadata(
    payload: *const GoRSocketPayload,
    buffer: *mut u8,
    buffer_len: size_t,
) -> size_t {
    ...
    unsafe {
        if let Some(metadata) = (*payload).inner.metadata() {
            let copy_len = std::cmp::min(metadata.len(), buffer_len);
            std::ptr::copy_nonoverlapping(metadata.as_ptr(), buffer, copy_len);
            copy_len
        } else {
            0
        }
...
#[no_mangle]
pub extern "C" fn rsocket_go_payload_free(payload: *mut GoRSocketPayload) {
    if !payload.is_null() {
        unsafe {
